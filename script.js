...
function getChecked(allowedValues) {
...
function futureIndicative(v, genus, index) {
  ...
  if (v.type === "a" || v.type === "e") {
    ...
    return s + (genus === "aktiv" ? aktiv[index] : passiv[index]);
  }
...
function formFor(v, modus, tempus, genus, index) {
  ...
  if (genus === "passiv" && tempus === "perfekt") {
    const aux = modus === "indikativ" ? ESSE.indikativ.praesens : ESSE.konjunktiv.praesens;
    return pppForm(v, index) + " " + aux[index];
  }

  if (genus === "passiv" && tempus === "plusquamperfekt") {
    const aux = modus === "indikativ" ? ESSE.indikativ.imperfekt : ESSE.konjunktiv.imperfekt;
    return pppForm(v, index) + " " + aux[index];
  }

  if (modus === "indikativ") {
    if (tempus === "praesens") return presentIndicative(v, genus, index);
    ...
    if (tempus === "futur1") return futureIndicative(v, genus, index);
  }

  if (modus === "konjunktiv") {
    if (tempus === "futur1") return null;

    if (tempus === "praesens") return presentSubjunctive(v, genus, index);
...
function generate() {
  const modi = getChecked(["indikativ", "konjunktiv"]);
  const tempora = getChecked(["praesens", "imperfekt", "perfekt", "plusquamperfekt", "futur1"]);
  const genera = getChecked(["aktiv", "passiv"]);

  if (modi.length === 0 || tempora.length === 0 || genera.length === 0) {
    document.getElementById("task").innerText = "Bitte mindestens eine Auswahl pro Bereich setzen.";
    return;
  }
  ...
  for (const modus of modi) {
    for (const tempus of tempora) {
      if (modus === "konjunktiv" && tempus === "futur1") continue;
      for (const genus of genera) {
        possible.push({ modus, tempus, genus });
      }
    }
  }

  if (possible.length === 0) {
    document.getElementById("task").innerText = "Diese Auswahl enthält keine gültige Kombination.";
    return;
  }

  const selection = random(possible);
  const verb = random(VERBS);
  const index = Math.floor(Math.random() * 6);
  const solution = formFor(verb, selection.modus, selection.tempus, selection.genus, index);

  current = {
    solution,
    label: `${verb.infinitive} – ${PERSONS[index]} ${selection.modus} ${selection.tempus} ${selection.genus}`
  };
...
function check() {
  ...
  if (input === current.solution) {
    ...
    document.getElementById("feedback").innerHTML = `<span class='wrong'>✖ Falsch. Richtig: ${current.solution}</span>`;
  }
